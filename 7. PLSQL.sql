/*
1. PL/SQL : Procedure Language; 오라클의 SQL 전용 언어
    - 프로시저/함수/트리거 만드는 언어
        cf. 함수; 프로그램 실행 단위
            - FUNCTION : 리턴형을 가지는 함수(내장함수)
            - PROCEDURE : 리턴형이 없는 함수(주로, 있어도 됨) -> 기능만 수행
            - TRIGGER : 자동 처리 
    1) 형식
        DECLARE
            선언부 --변수(지역변수, 매개변수) 선언
    ...
    2) 변수
        ...
        - %TYPE : 실제 테이블 변수의 데이터형 가져옴
            - 테이블명.컬럼명%TYPE
                ex. emp.empno%TYPE;
        - %ROWTYPE : 실제 테이블 변수 전체의 모든 데이터형 가져옴(VO)
            - 테이블명%ROWTYPE
                ex. emp%ROWTYPE;
            - 테이블 1개만 사용 가능(보안에 뛰어남), 조인 불가
        - RECORD : 사용자 정의 데이터형 
            - 테이블 여러개 묶어서 사용할 때(조인, 서브쿼리) 주로 사용
            - TYPE 레코드명 IS RECORD(
                변수명 데이터형..
              );
              레코드변수명 레코드명;
        - CURSOR : 전체 ROW에 대한 데이터 가져올 때 사용하는 변수 -> 자바의 ResultSet
            - CURSOR 커서명 IS
                SELECT~
            - SELECT문 처리 결과 커서에 저장(뷰와 비슷한 기능)  
            ...
            - 사용 순서
                ...
                3. 데이터 추출
                    LOOP
                        FETCH 커서명 IN 변수명..
                        EXIT WHEN
                            커서명%NOTFOUND | 커서명%FOUND | 커서명%COUNT
                4. 커서 닫기
                    CLOSE 커서명;
    3) 제어문
        (1) 조건문
